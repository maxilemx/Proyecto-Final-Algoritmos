...
@st.cache_data
def calcular_codo(clusters_df):
    """Recalcula la INERCIA de K-Means para k=1..10 (método del codo).
    Usa las mismas 4 variables y el mismo estandarizado que el clustering
    real, así que reproduce el codo que justifica k=4. Cacheado: corre
    una sola vez aunque el usuario interactúe con el dashboard."""
    from sklearn.preprocessing import StandardScaler
    from sklearn.cluster import KMeans
    feats = ["recencia_dias", "total_gastado", "antiguedad_dias", "edad"]
    X = StandardScaler().fit_transform(clusters_df[feats].values)
    ks = list(range(1, 11))
    inercias = [KMeans(n_clusters=k, random_state=42, n_init=10).fit(X).inertia_
                for k in ks]
    return ks, inercias


def grafica_codo(ks, inercias):
    """Línea del método del codo, resaltando k=4 (el 'codo')."""
    fig = go.Figure(go.Scatter(
        x=ks, y=inercias, mode="lines+markers",
        line=dict(color=CAFE, width=2), marker=dict(size=8, color=CAFE),
        hovertemplate="k = %{x}<br>Inercia: %{y:,.0f}<extra></extra>",
    ))
    # Círculo rojo + anotación sobre k=4
    i4 = inercias[3]
    fig.add_trace(go.Scatter(
        x=[4], y=[i4], mode="markers", showlegend=False,
        marker=dict(size=22, color="rgba(0,0,0,0)", line=dict(color=ROJO, width=3)),
        hovertemplate="El codo: k = 4<extra></extra>",
    ))
    fig.add_annotation(x=4, y=i4, text="<b>k = 4</b><br>(el codo)", showarrow=True,
                       arrowhead=2, arrowcolor=ROJO, arrowwidth=1.5,
                       ax=70, ay=-45, font=dict(color=ROJO, size=13),
                       bgcolor="rgba(255,255,255,0.85)", bordercolor=ROJO, borderwidth=1)
    _layout_cafe(fig, alto=360, titulo_x="Número de segmentos (k)",
                 titulo_y="Inercia (dispersión dentro de los grupos)")
    fig.update_xaxes(dtick=1)
    return fig
...
perfil = clientes.groupby('cluster').agg(
    ...
    # ------------------------------------------------------------------
    # Método del codo: ¿por qué k=4? (justifica el número de segmentos)
    # ------------------------------------------------------------------
    st.markdown("---")
    st.subheader("📐 ¿Por qué exactamente 4 segmentos? El método del codo")
    st.caption(
        "Antes de mostrar los grupos: ¿por qué 4 y no 3 o 5? Corremos K-Means con "
        "distinto número de grupos (k) y medimos la 'inercia' (qué tan apretados quedan). "
        "Buscamos el 'codo': el punto donde sumar más grupos ya casi no mejora."
    )
    _feats_codo = {"recencia_dias", "total_gastado", "antiguedad_dias", "edad"}
    if clusters_df is not None and _feats_codo.issubset(clusters_df.columns):
        ks_codo, inercias_codo = calcular_codo(clusters_df)
        mostrar(grafica_codo(ks_codo, inercias_codo))
        st.caption(
            "📖 **Cómo leerlo:** la línea cae muchísimo de k=1 a k=4 (cada grupo nuevo "
            "ordena mucho), pero a partir de k=4 se aplana: un 5º o 6º grupo casi no "
            "reduce la dispersión y solo complica la historia. Por eso elegimos **k = 4**."
        )
        bloque_codigo("Método del codo", '''# ¿Cuántos grupos? Probamos k=1..10 y medimos la inercia.
from sklearn.preprocessing import StandardScaler
from sklearn.cluster import KMeans

feats = ["recencia_dias", "total_gastado", "antiguedad_dias", "edad"]
X = StandardScaler().fit_transform(clientes[feats].values)

inercias = []
for k in range(1, 11):
    km = KMeans(n_clusters=k, random_state=42, n_init=10).fit(X)
    inercias.append(km.inertia_)   # inercia = suma de distancias a su centroide

# Se grafica inercia vs k y se busca el "codo": donde la curva deja de
# bajar fuerte. Aquí ocurre en k=4 -> ese es el número de segmentos.''')
    else:
        st.info("Para ver el método del codo se necesita output/clientes_clusters.csv "
                "con las columnas de recencia, gasto, antigüedad y edad.")
